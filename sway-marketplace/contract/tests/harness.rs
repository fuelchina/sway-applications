...
async fn can_set_owner() {
    ...
    let owner_result = instance.with_account(wallet_1.clone()).methods()
    .initialize_owner().call().await.unwrap();

  assert!(Identity::Address(wallet_1.address().into()) == owner_result.value, "Owner is not set correctly")
